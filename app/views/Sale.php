...
<?php
    ...
    // Set the sale end time in session if not already set
    if (!isset($_SESSION['sale_end_time'])) {
        // Set end time to 2 days from now
        $endTime = new DateTime();
        $endTime->modify('+2 days');
        $_SESSION['sale_end_time'] = $endTime->format('Y-m-d H:i:s');
    }

    // Get the sale end time from session
    $saleEndTime = isset($_SESSION['sale_end_time']) ? $_SESSION['sale_end_time'] : (new DateTime())->modify('+2 days')->format('Y-m-d H:i:s');
?>

<section class="sale-banner">
    <div class="banner-content">
        <h1>🔥 HUGE SALE 🔥</h1>
        <p>Limited time offers on our most popular items - 50% OFF AND MORE</p>
        <div class="sale-countdown" data-end-time="<?php echo $saleEndTime; ?>">
            <div class="countdown-item" data-unit="days">
                <span class="time" id="days">00</span><span>Days</span>
            </div>
            <div class="countdown-item" data-unit="hours">
                <span class="time" id="hours">00</span><span>Hours</span>
            </div>
            <div class="countdown-item" data-unit="minutes">
                <span class="time" id="minutes">00</span><span>Minutes</span>
            </div>
            <div class="countdown-item" data-unit="seconds">
                <span class="time" id="seconds">00</span><span>Seconds</span>
            </div>
        </div>
    </div>
</section>
